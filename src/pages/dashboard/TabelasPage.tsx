        ...
        <div className="px-5 py-5 space-y-3">
          ...
          {selected.length > 0 && (
            <div className="flex gap-2 pt-1">
              ...
              {confirmEmail ? (
                <div className="flex-1 rounded-xl border border-blue-500/30 bg-blue-500/5 p-2.5 text-center">
                  <p className="text-xs text-gray-300 mb-2">
                    Enviar para {withEmail.map(c => c.name).join(', ')}?
                  </p>
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => { setConfirmEmail(false); handleEmail() }} className="flex-1 py-1.5 bg-blue-500 text-white rounded-lg text-xs font-semibold">Sim, enviar</button>
                  </div>
                </div>
              ) : (
                <button
